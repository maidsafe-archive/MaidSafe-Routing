...
MatrixChange::MatrixChange(const MatrixChange& other)
    : kNodeId_(other.kNodeId_),
      kOldMatrix_(other.kOldMatrix_),
      kNewMatrix_(other.kNewMatrix_),
      kLostNodes_(other.kLostNodes_) {}
...
MatrixChange::MatrixChange(NodeId this_node_id, const std::vector<NodeId>& old_matrix,
    ...
    : kNodeId_(std::move(this_node_id)),
      ...
      kLostNodes_([this]()->std::vector<NodeId> {
        ...
        std::set_difference(kOldMatrix_.begin(), kOldMatrix_.end(), kNewMatrix_.begin(),
                            kNewMatrix_.end(), std::back_inserter(lost_nodes),
                            [this](const NodeId & lhs, const NodeId & rhs) {
          return NodeId::CloserToTarget(lhs, rhs, kNodeId_);
        });
        return lost_nodes;
...
CheckHoldersResult MatrixChange::CheckHolders(const NodeId& target) const {
  ...
  std::partial_sort_copy(kOldMatrix_.begin(), kOldMatrix_.end(), old_holders.begin(),
                         old_holders.end(), [target](const NodeId & lhs, const NodeId & rhs) {
    return NodeId::CloserToTarget(lhs, rhs, target);
  });
  std::partial_sort_copy(kNewMatrix_.begin(), kNewMatrix_.end(), new_holders.begin(),
                         new_holders.end(), [target](const NodeId & lhs, const NodeId & rhs) {
    return NodeId::CloserToTarget(lhs, rhs, target);
  });
  std::sort(lost_nodes.begin(), lost_nodes.end(), [target](const NodeId & lhs, const NodeId & rhs) {
    return NodeId::CloserToTarget(lhs, rhs, target);
  });
  ...
  holders_result.proximity_status =
      GetProximalRange(target, kNodeId_, kNodeId_, kRadius_, new_holders);
  ...
  std::set_intersection(old_holders.begin(), old_holders.end(), lost_nodes.begin(),
                        lost_nodes.end(), std::back_inserter(holders_result.old_holders),
                        [target](const NodeId & lhs, const NodeId & rhs) {
    return NodeId::CloserToTarget(lhs, rhs, target);
  });

  // New holders = All new holders - Old holders
  std::set_difference(new_holders.begin(), new_holders.end(), old_holders.begin(),
                      old_holders.end(), std::back_inserter(holders_result.new_holders),
                      [target](const NodeId & lhs, const NodeId & rhs) {
    return NodeId::CloserToTarget(lhs, rhs, target);
  });
  return holders_result;
