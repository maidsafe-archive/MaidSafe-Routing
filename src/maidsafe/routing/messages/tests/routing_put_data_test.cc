...
TEST(PutDataTest, BEH_SerialiseParse) {
  ...
  EXPECT_EQ(put_data_before.part.size(), put_data_after.part.size());
  EXPECT_TRUE(std::equal(put_data_before.part.begin(), put_data_before.part.end(),
                         put_data_before.part.begin()));
}
