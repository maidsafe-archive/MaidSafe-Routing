...
TEST(RoutingTableTest, BEH_OrderedGroupChange) {
  ...
  std::vector<NodeId> close_nodes(routing_table.group_matrix_.GetConnectedPeers());
  ...
  for (uint16_t i(0); i < std::min(expected_close_nodes.size(), close_nodes.size()); ++i)
    EXPECT_EQ(expected_close_nodes.at(i), close_nodes.at(i));
}
...
TEST(RoutingTableTest, BEH_ReverseOrderedGroupChange) {
  ...
  std::vector<NodeId> close_nodes2(routing_table.group_matrix_.GetConnectedPeers());
  ...
  for (uint16_t i(0); i < std::min(expected_close_nodes2.size(), close_nodes2.size()); ++i)
    EXPECT_EQ(expected_close_nodes2.at(i), close_nodes2.at(i));
...
TEST(RoutingTableTest, FUNC_CheckMockSendGroupChangeRpcs) {
  ...
  RoutingTable routing_table_1(fob_1, false);
  ...
  fob_2.identity = Identity(extra_nodes.at(index_2).node_id.string());
  RoutingTable routing_table_2(fob_2, false);
  ...
  (const std::vector<NodeInfo> nodes) {
    ...
    for (auto node_info : nodes) {
      if (node_info.node_id == routing_table_2.kNodeId())
        ...
        close_nodes.push_back(node_info.node_id);
    }
...
TEST(RoutingTableTest, BEH_GroupUpdateFromConnectedPeer) {
  ...
  for (auto node_info : nodes) {
    ...
    for (uint16_t i(0); i < std::min(new_row_entries.size(), row_contents.size()); ++i)
      EXPECT_EQ(row_contents.at(i), new_row_entries.at(i));
