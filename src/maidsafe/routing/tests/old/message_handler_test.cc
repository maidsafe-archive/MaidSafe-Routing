...
class MessageHandlerTest : public testing::Test {
 public:
  MessageHandlerTest()
    ...
    network_network_.reset(new NetworkUtils(Address, asio_service_));
    ...
    close_info_.id = GenerateUniqueRandomId(table_->kAddress(), 20);
...
TEST_F(MessageHandlerTest, BEH_HandleInvalidMessage) {
  ...
  message.set_SourceAddress(Address().string());
...
TEST_F(MessageHandlerTest, BEH_HandleRelay) {
  ...
  {  // Handle direct relay request to self
    ...
    message.set_DestinationAddress(table_->kAddress().string());
  ...
  {  // Handle direct relay request to other not in routing table
    Address DestinationAddress(GenerateUniqueRandomId(close_info_.id, static_cast<unsigned int>(4)));
    EXPECT_CALL(*network_,
                SendToClosestNode(testing::AllOf(
                    testing::Property(&protobuf::Message::DestinationAddress, destination_id.string()),
                    testing::Property(&protobuf::Message::SourceAddress, table_->kAddress().string()))))
        .Times(1)
        .RetiresOnSaturation();
    EXPECT_CALL(*network_, SendToDirect(testing::_, testing::_, testing::_)).Times(0);
    EXPECT_CALL(*service_, FindNodes(testing::_)).Times(0);
    ...
    message.set_relay_id(relay_id);
    message.set_hops_to_live(1);
    message.set_DestinationAddress(destination_id.string());
    message_handler.HandleMessage(message);
  }
  {  // Handle direct relay request to other in routing table
    EXPECT_CALL(*network_,
                SendToClosestNode(testing::AllOf(
                    testing::Property(&protobuf::Message::DestinationAddress, close_info_.id.string()),
                    testing::Property(&protobuf::Message::SourceAddress, table_->kAddress().string()))))
        .Times(1)
        .RetiresOnSaturation();
    EXPECT_CALL(*network_, SendToDirect(testing::_, testing::_, testing::_)).Times(0);
    ...
    message.set_relay_id(relay_id);
    message.set_hops_to_live(1);
    message.set_DestinationAddress(close_info_.id.string());
    message_handler.HandleMessage(message);
  }
}
...
TEST_F(MessageHandlerTest, DISABLED_BEH_HandleGroupMessage) {
  ...
  /*{  // Handle group message to self
    ...
    EXPECT_CALL(*network_, SendToClosestNode(testing::AllOf(
                                           testing::Property(&protobuf::Message::DestinationAddress,
                                                             table_->kAddress()),
                                           testing::Property(&protobuf::Message::SourceAddress,
                                                             table_->kAddress()))))
    ...
    EXPECT_CALL(*response_handler_, Connect(testing::_)).Times(0);
    EXPECT_CALL(*response_handler_, ConnectSuccess(testing::_)).Times(0);
    message.set_SourceAddress(table_->kAddress());
    message.set_DestinationAddress(table_->kAddress());
    message_handler.HandleMessage(message);
  }*/
  for (int i(0); i < 3; ++i) {
  ...
  {  // Handle group message to node in routing table's closest
    ...
    EXPECT_CALL(*network_, SendToClosestNode(testing::_)).Times(0);
    EXPECT_CALL(*network_,
                SendToDirect(testing::AllOf(testing::Property(&protobuf::Message::DestinationAddress,
    ...
    EXPECT_CALL(*network_,
        ...
        .Times(1)
        .RetiresOnSaturation();
    //    EXPECT_CALL(*table_, IsAddressInGroupRange(testing::_, testing::_)).Times(1);
    EXPECT_CALL(*service_, FindNodes(testing::_)).Times(0);
    EXPECT_CALL(*service_, Ping(testing::_)).Times(0);
    EXPECT_CALL(*service_, Connect(testing::_)).Times(0);
    EXPECT_CALL(*response_handler_, FindNodes(testing::_)).Times(0);
    ...
    message.set_SourceAddress(RandomString(64));
    message.set_DestinationAddress(close_info_.id.string());
    message_handler.HandleMessage(message);
  }
  {  // Handle group message to node not in routing table's closest
    ...
    message.set_client_node(true);
    message.set_ack_id(RandomInt32());
    Address SourceAddress(RandomString(Address::kSize));
    Address DestinationAddress(GenerateUniqueRandomId(close_info_.id, 4));
    EXPECT_CALL(*network_,
                SendToClosestNode(testing::AllOf(
                    testing::Property(&protobuf::Message::DestinationAddress, destination_id.string()),
                    testing::Property(&protobuf::Message::SourceAddress, source_address.string()))))
        .Times(1)
        .RetiresOnSaturation();
    EXPECT_CALL(*table_, IsAddressInGroupRange(testing::_, testing::_)).Times(0);
    EXPECT_CALL(*network_, SendToDirect(testing::_, testing::_, testing::_)).Times(0);
    EXPECT_CALL(*service_, FindNodes(testing::_)).Times(0);
    EXPECT_CALL(*service_, Ping(testing::_)).Times(0);
    EXPECT_CALL(*service_, Connect(testing::_)).Times(0);
    ...
    EXPECT_CALL(*response_handler_, ConnectSuccess(testing::_)).Times(0);
    message.set_hops_to_live(1);
    message.set_SourceAddress(source_address.string());
    message.set_DestinationAddress(destination_id.string());
    message_handler.HandleMessage(message);
  }
  {  // Handle FindNodes non-relay group message to destination closest to us
    ...
    message.set_ack_id(RandomInt32());
    Address SourceAddress(RandomString(Address::kSize));
    Address DestinationAddress(GenerateUniqueRandomId(table_->kNodeId(), 4));
    auto closest_nodes(table_->GetClosestNodes(table_->kAddress(), 4));
    EXPECT_CALL(*network_, SendToClosestNode(testing::_)).Times(0);
    EXPECT_CALL(*network_,
                SendToDirect(testing::AllOf(testing::Property(&protobuf::Message::DestinationAddress,
                                                              closest_nodes.at(0).id.string()),
  ...
  {  // Handle Node level non-relay group message to destination closest to us
    ...
    EXPECT_CALL(*network_,
                SendToClosestNode(testing::AllOf(
                    ...
                    testing::Property(&protobuf::Message::SourceAddress, table_->kAddress().string()),
                    testing::Property(&protobuf::Message::DestinationAddress, SourceAddress.string()))))
        .Times(1)
        .RetiresOnSaturation();
    EXPECT_CALL(*network_,
                SendToDirect(testing::AllOf(testing::Property(&protobuf::Message::DestinationAddress,
  ...
  {  // Handle Node level relay group message to destination closest to us
    ...
    EXPECT_CALL(*network_,
        ...
        .Times(1)
        .RetiresOnSaturation();
    EXPECT_CALL(*table_, IsAddressInGroupRange(testing::_, result))
...
TEST_F(MessageHandlerTest, BEH_ClientRoutingTable) {
  ...
  table_.reset(new MockRoutingTable(true, Address(maid.name()->string()), keys));
  table_->AddNode(close_info_);
  MessageHandler message_handler(*table_, *ntable_, *network_, timer_, *network_network_,
                                 asio_service_);
  message_handler.service_ = service_;
  ...
  message.set_client_node(true);
  message.set_SourceAddress(RandomString(64));
  message.set_DestinationAddress(maid.name()->string());
  message.add_data("DATA");
  message_handler.set_message_and_caching_functor(message_and_caching_functor_);
  {  // Handle node level request to this node
  ...
  {  // Handle routing Connect request to this node
    ...
    connect_message.set_SourceAddress(RandomString(64));
    connect_message.set_DestinationAddress(maid.name()->string());
