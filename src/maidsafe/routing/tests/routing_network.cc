...
testing::AssertionResult GenericNetwork::SendDirect(const size_t& repeats) {
  ...
  size_t expected_good_futures(server_size * (server_size - 1 + client_size) * repeats);
...
testing::AssertionResult GenericNetwork::SendGroup(const NodeId& target_id,
  ...
  assert(static_cast<long>(10 * repeats) > 0); // NOLINT (Fraser)
  std::string data(RandomAlphaNumericString((2 ^ 10) * 256));

  std::vector<std::vector<std::future<std::string>>> futures;
  for (size_t index = 0; index < repeats; ++index) {
    futures.push_back(this->nodes_[source_index]->SendGroup(target_id, data, false));
  }

  size_t bad_futures_count(0);

  std::vector<std::vector<std::string>> replies(repeats);
  ...
  std::vector<NodeId> expected_group(this->GetGroupForId(target_id));
  bool repliers_ok(true);
  for (size_t index(0); index < repeats; ++index) {
    for (auto reply : replies.at(index)) {
      try {
        ...
        if (std::find_if(expected_group.begin(),
                         expected_group.end(),
                         [&](const NodeId& target_id) { return target_id == replier; })
            == expected_group.end()) {
          repliers_ok = false;
  ...
  if (!repliers_ok) {
    LOG(kInfo) << "Expected group nodes for ID " << DebugId(target_id) << ":";
...
testing::AssertionResult GenericNetwork::SendDirect(const NodeId& node_id,
