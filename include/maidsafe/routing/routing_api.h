...
class Routing {
 public:
  ...
  Routing(boost::filesystem::path bootstrap_file_path = boost::filesystem::path());
  ...
  explicit Routing(const FobType& fob,
                   boost::filesystem::path bootstrap_file_path = boost::filesystem::path())
      : pimpl_() {
    asymm::Keys keys;
...
template <>
Routing::Routing(const NodeId& node_id, boost::filesystem::path bootstrap_file_path);
